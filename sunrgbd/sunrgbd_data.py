...
def extract_sunrgbd_data(
    ...
    for data_idx in data_idx_list:
        ...
        if save_votes:
            if shapenet:
                stop = 1
                pc_load = np.load(os.path.join(output_folder, "%06d_pc.npz" % (data_idx)))
                pc_upright_depth_subsampled = pc_load["pc"]
...
def get_votes_shapenet(output_folder, scene_list=[], num_point=50000, Petra=True):

    if scene_list:
        data_idx_list = [int(line.rstrip()) for line in open(scene_list)]
    else:
        data_idx_list = [int(x.stem[0:6]) for x in Path(output_folder).rglob("*.npy")]

    for data_idx in sorted(data_idx_list, reverse=False):
        print("------------- ", data_idx)

        pc_load = np.load(os.path.join(output_folder, "%06d_pc.npz" % (data_idx)))
        pc_upright_depth_subsampled = pc_util.random_sampling(pc_load["pc"], num_point)
        np.savez_compressed(os.path.join(output_folder, "%06d_pc.npz" % (data_idx)), pc=pc_upright_depth_subsampled)

        N = pc_upright_depth_subsampled.shape[0]

        bboxes_load = np.load(os.path.join(output_folder, "%06d_bbox.npy" % (data_idx)))
        ...
        for iBox in range(0, bboxes_load.size, 8):
            obb = bboxes_load[iBox : iBox + 8]
            obb_list.append(obb)

            try:
                # Find all points in this object's OBB
                box3d_pts_3d = sunrgbd_utils.my_compute_box_3d(obb[0:3], obb[3:6], obb[6])
                pc_in_box3d, inds = sunrgbd_utils.extract_pc_in_box3d(pc_upright_depth_subsampled, box3d_pts_3d)
                # Assign first dimension to indicate it is in an object box
                point_votes[inds, 0] = 1
                # Add the votes (all 0 if the point is not in any object's OBB)
                votes = np.expand_dims(obb[0:3], 0) - pc_in_box3d[:, 0:3]
                sparse_inds = indices[inds]  # turn dense True,False inds to sparse number-wise inds
                for i in range(len(sparse_inds)):
                    j = sparse_inds[i]
                ...
                print("ERROR ----", data_idx)

            np.savez_compressed(os.path.join(output_folder, "%06d_votes.npz" % (data_idx)), point_votes=point_votes)

            stop = 1

        np.save(os.path.join(output_folder, "%06d_bbox" % (data_idx)), obb_list)
        stop = 1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--shapenet_data", action="store_true", help="Generate votes.")
    parser.add_argument("--scenes_list", help="Path to txt file containg scenes list")
    ...
    if args.shapenet_data:
        get_votes_shapenet(args.output_folder, num_point=50000)
