...
parser.add_argument('--npz', action='store_true', default=False, help='Set this flag if scene is saved as npz file')
...
if __name__=='__main__':
    ...
    if FLAGS.dataset == 'sunrgbd':
        ...
        sys.path.append(os.path.join(ROOT_DIR, 'output'))
        from sunrgbd_detection_dataset import DC # dataset config
        checkpoint_path = FLAGS.checkpoint_path
        pc_path = FLAGS.scene_path
    else:
    ...
    if(FLAGS.npz):
        # Point cloud as npz
        point_cloud = np.load(pc_path)['pc']
        # point_cloud = np.load('/media/mateja/ubuntu_storage/Datasets/Kinect_scenes_K1-2/supporting_plane/pc.npz')['pc']
        # point_cloud = np.load('/media/mateja/ubuntu_storage/Git_repos/votenet/shapenet/pc_bbox_votes_50k_val/000012_pc.npz')['pc']
    else:
        # Point cloud as ply
        # point_cloud = read_ply(pc_path)
        mesh = PlyData.read(pc_path)
        point_cloud = np.stack(
            (np.array(mesh["vertex"]["x"]), np.array(mesh["vertex"]["y"]), np.array(mesh["vertex"]["z"])), axis=1
        )
