...
from ap_helper_mateja import APCalculator, parse_predictions, parse_groundtruths
...
parser.add_argument(
    "--lr_decay_steps", default="80,120,160", help="When to decay the learning rate (in epochs) [default: 80,120,160]"
)
parser.add_argument("--lr_decay_rates", default="0.1,0.1,0.1", help="Decay rates for lr decay [default: 0.1,0.1,0.1]")
parser.add_argument("--no_height", action="store_true", help="Do NOT use height signal in input.")
parser.add_argument("--use_color", action="store_true", help="Use RGB color in input.")
parser.add_argument("--use_sunrgbd_v2", action="store_true", help="Use V2 box labels for SUN RGB-D dataset")
parser.add_argument("--use_shapenet", action="store_true", help="Use SHAPENET labels for SUN RGB-D dataset")
...
if FLAGS.dataset == "sunrgbd":
    ...
    TRAIN_DATASET = ScannetDetectionDataset(
        "train", num_points=NUM_POINT, augment=True, use_color=FLAGS.use_color, use_height=(not FLAGS.no_height)
    )
...
TRAIN_DATALOADER = DataLoader(
    TRAIN_DATASET, batch_size=BATCH_SIZE, shuffle=True, num_workers=4, worker_init_fn=my_worker_init_fn
)
TEST_DATALOADER = DataLoader(
    TEST_DATASET, batch_size=BATCH_SIZE, shuffle=True, num_workers=4, worker_init_fn=my_worker_init_fn
)
...
def evaluate_one_epoch():
    stat_dict = {}  # collect statistics
    ap_calculator = APCalculator(ap_iou_thresh=FLAGS.ap_iou_thresh, class2type_map=DATASET_CONFIG.class2type)
    ...
    ## MATEJA-EVAL
    metrics_dict, metrics_dist_mateja = ap_calculator.compute_metrics()
...
def train(start_epoch):
    ...
    ## MATEJA
    # f= open("/media/mateja/ubuntu_storage/Git_repos/votenet/output/log_votenet/log_loss_mateja.txt","w")
    # f.write("center_loss\t heading_class_loss\t heading_residual_normalized_loss\t size_class_loss\t size_residual_normalized_loss\t sem_cls_loss\n\n")
    # f.close()
    ##

    for epoch in range(start_epoch, MAX_EPOCH):
        ## MATEJA
        # f= open("/media/mateja/ubuntu_storage/Git_repos/votenet/output/log_votenet/log_loss_mateja.txt","a")
        # f.write('\n**** EPOCH %03d ****\n' % (epoch))
        # f.close()
        ##
