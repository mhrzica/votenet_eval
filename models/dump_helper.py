...
# MATEJA
# DUMP_CONF_THRESH = 0.9
DUMP_CONF_THRESH = 0.75
    ...
    for i in range(batch_size):
        ...
        if np.sum(objectness_prob > DUMP_CONF_THRESH) > 0:
            ...
            if len(obbs) > 0:
                ...
                # MATEJA
                bbox_conf_list = objectness_prob[
                    np.logical_and(objectness_prob > DUMP_CONF_THRESH, pred_mask[i, :] == 1)
                ]
                objectness_prob_path = os.path.join(dump_dir, "%06d_objectness_prob.npy" % (idx_beg + i))
                np.save(objectness_prob_path, bbox_conf_list)
