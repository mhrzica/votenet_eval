...
def compute_box_and_sem_cls_loss(end_points, config):
    ...
    ## MATEJA
    # results = [center_loss.cpu().detach().numpy(), heading_class_loss.cpu().detach().numpy(), heading_residual_normalized_loss.cpu().detach().numpy(), size_class_loss.cpu().detach().numpy(), size_residual_normalized_loss.cpu().detach().numpy(), sem_cls_loss.cpu().detach().numpy()]
    # joined = str(results[0]) + '\t ' + str(results[1]) + '\t ' + str(results[2]) + '\t ' + str(results[3]) + '\t ' + str(results[4]) + '\t ' + str(results[5]) + '\n'
