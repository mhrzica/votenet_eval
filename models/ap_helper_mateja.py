...
from nms_mateja import nms_2d_faster, nms_3d_faster, nms_3d_faster_samecls
...
def parse_groundtruths(end_points, config_dict):
    ...
    for i in range(bsize):
        for j in range(K2):
            ...
            ## MATEJA-EVAL
            if box_size.sum() < 1.0e-6:
                # 3.0572891218660914e-08
                stop = 1
                # continue
...
class APCalculator(object):
    ...
    def compute_metrics(self):
        ...
        ret_dict_mateja = {}
        for key in sorted(ap.keys()):
            ret_dict_mateja["%s precision" % (clsname)] = prec[key][-1]
            ret_dict_mateja["%s recall" % (clsname)] = rec[key][-1]
            ret_dict_mateja["mAP"] = np.mean(list(ap.values()))
            ret_dict_mateja["average recall"] = np.mean(rec_list)

        return ret_dict, ret_dict_mateja
        # return ret_dict
